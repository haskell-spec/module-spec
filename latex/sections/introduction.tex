...
We use S-expressions for every intermediate representation, and when we define some concrete syntax of S-expressions we use $\terminal{terminal}$ for symbols/terminals and $\nonterminal{nonterminal}$ for non-terminal symbols of the grammar.
Furthermore, we use $\overline{\nonterminal{nonterminal}}$ as a shorthand for $(\nonterminal{nonterminal}_1 \ldots \nonterminal{nonterminal}_n)$ where $n$ can be 0 and the list empty.